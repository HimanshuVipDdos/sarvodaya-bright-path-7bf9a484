...
import { Send, Trash2, UserRound, Shield } from "lucide-react";
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        {messages.map((m) => (
          <div key={m.id} className={`group flex items-start justify-between gap-2 text-sm ${m.is_moderator ? "rounded-xl bg-primary/5 p-1.5 -m-1.5" : ""}`}>
            <div className="min-w-0">
              ...
              <span className="break-words ml-1">{m.message}</span>
            </div>
            {canModerate && (
              <button
                onClick={() => remove(m.id)}
