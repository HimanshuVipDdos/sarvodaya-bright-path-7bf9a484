...
type TestRow = {
  ...
  negative_marking: boolean; negative_marks: number; question_limit: number | null;
};
...
const emptyForm = {
  ...
  negative_marking: false, negative_marks: 0.25, question_limit: "" as string | number,
};
...
function CbtAdmin() {
  ...
  function openEdit(t: TestRow) {
    ...
    setForm({
      ...
      question_limit: t.question_limit ?? "",
    });
    setOpen(true);
  }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        negative_marks: form.negative_marking ? (Number(form.negative_marks) || 0) : 0,
        question_limit: form.question_limit === "" ? null : Number(form.question_limit) || null,
      };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div>
              <Label className="text-xs">Question limit (optional)</Label>
              <Input
                type="number" min={1} placeholder="Leave blank to use all questions"
                value={form.question_limit}
                onChange={(e) => setForm((f) => ({ ...f, question_limit: e.target.value === "" ? "" : Number(e.target.value) }))}
              />
              <p className="mt-1 text-[11px] text-muted-foreground">
                If set, each student gets this many questions picked at random from the full question bank (same set kept if they resume).
              </p>
            </div>
            <div className="rounded-xl border border-border/60 p-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">Deduct marks for wrong answers. Off by default.</p>
                </div>
                <Switch checked={form.negative_marking} onCheckedChange={(v) => setForm((f) => ({ ...f, negative_marking: v }))} />
              </div>
              {form.negative_marking && (
                <div className="mt-3">
                  <Label className="text-xs">Marks deducted per wrong answer</Label>
                  <Input
                    type="number" step="0.05" min={0}
                    value={form.negative_marks}
                    onChange={(e) => setForm((f) => ({ ...f, negative_marks: Number(e.target.value) }))}
                  />
                </div>
              )}
            </div>
