  ...
  return (
    <Section>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold">Question-wise Review</h2>
        <div className="flex gap-1 rounded-full bg-muted p-1 text-xs">
          <button
            onClick={() => setFilter("all")}
            className={cn("rounded-full px-3 py-1 font-medium transition", filter === "all" ? "bg-background shadow-sm" : "text-muted-foreground")}
          >
            All ({data.all_answers.length})
          </button>
          <button
            onClick={() => setFilter("wrong")}
            className={cn("rounded-full px-3 py-1 font-medium transition", filter === "wrong" ? "bg-background shadow-sm" : "text-muted-foreground")}
          >
            Mistakes only ({data.mistakes.length})
          </button>
        </div>
      </div>

      <div className="space-y-4">
        {data.all_answers.length === 0 && (
          <div className="glass-strong rounded-3xl p-8 text-center text-sm text-muted-foreground">
            No answer data for this attempt.
          </div>
        )}
        {(filter === "all" ? data.all_answers : data.all_answers.filter((a: any) => !a.is_correct)).map((m: any, i: number) => (
