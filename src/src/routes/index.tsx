      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <motion.div 
            ...
            className="absolute top-[20%] -right-[10%] w-[40%] h-[60%] rounded-full bg-primary-glow/20 blur-[100px]" 
          />
        </div>
        ...
        >
          <div className="grid items-center gap-12 lg:grid-cols-2">
            ...
            >
              <motion.div variants={{ hidden: { opacity: 0, x: -30, filter: "blur(4px)" }, visible: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }}>
                <div className="inline-flex items-center gap-2 rounded-full glass px-4 py-2 text-xs font-semibold text-primary shadow-glow">
                  <Sparkles className="h-4 w-4" /> Premium coaching · Kasganj, UP
                </div>
              </motion.div>
              
              <motion.h1 variants={{ hidden: { opacity: 0, x: -30, filter: "blur(4px)" }, visible: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="mt-6 text-balance text-5xl font-extrabold tracking-tight sm:text-6xl md:text-7xl">
                Crack India's toughest{" "}
                <span className="text-gradient">competitive exams</span>{" "}
                with confidence.
              </motion.h1>
              
              <motion.p variants={{ hidden: { opacity: 0, x: -30, filter: "blur(4px)" }, visible: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="mt-6 max-w-xl text-lg text-muted-foreground sm:text-xl">
                Live classes, recorded lectures, daily practice problems and mock tests —
                taught with discipline by <strong className="text-foreground">{SITE.owner}</strong> and a senior faculty team.
              </motion.p>
              
              <motion.div variants={{ hidden: { opacity: 0, x: -30, filter: "blur(4px)" }, visible: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="mt-8 flex flex-wrap gap-4">
                <Button asChild size="lg" className="rounded-full bg-gradient-to-br from-primary to-primary-glow px-8 py-6 text-base shadow-elegant hover:scale-105 transition-transform">
                  <Link to="/batches">
                    Explore Batches <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>
                <Button asChild size="lg" variant="ghost" className="rounded-full glass px-8 py-6 text-base hover-lift">
              ...
              <motion.div variants={{ hidden: { opacity: 0, x: -30, filter: "blur(4px)" }, visible: { opacity: 1, x: 0, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }} className="mt-12 grid max-w-md grid-cols-3 gap-4">
                {[
                  { k: "1K+", v: "Aspirants" },
                  { k: "500+", v: "Selections" },
                  { k: "13+", v: "Batches" },
                ].map((s) => (
                  <div key={s.v} className="glass rounded-2xl px-4 py-4 text-center hover-lift">
                    <div className="text-2xl font-black text-gradient">{s.k}</div>
                    <div className="mt-1 text-[11px] font-medium uppercase tracking-widest text-muted-foreground">{s.v}</div>
                  </div>
                ))}
              </motion.div>
            </motion.div>
            ...
            >
              <div className="glass-tint relative aspect-square overflow-hidden rounded-[3rem] p-6 sm:p-10 animate-float">
                <div className="absolute -right-16 -top-16 h-72 w-72 rounded-full bg-gradient-to-br from-primary to-primary-glow opacity-40 blur-[80px]" />
                <div className="absolute -bottom-20 -left-20 h-80 w-80 rounded-full bg-primary-glow/30 blur-[80px]" />
                ...
                <div className="relative grid h-full grid-cols-2 gap-4">
                  <motion.div
                    ...
                    className="glass-strong col-span-2 flex flex-col justify-center rounded-3xl p-5"
                  >
                    <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary">
                      ...
                      Live Class · UP Police
                    </div>
                    <div className="mt-2 text-base font-semibold">Reasoning — Coding/Decoding</div>
                    <div className="mt-3 flex items-center gap-2 text-sm font-medium text-muted-foreground">
                      <Calendar className="h-4 w-4" /> Today · 7:00 PM
                    </div>
                  </motion.div>

                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.65 }} className="glass-strong rounded-3xl p-5 flex flex-col justify-center items-center text-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 to-primary-glow/20 mb-3">
                      <Trophy className="h-6 w-6 text-primary" />
                    </div>
                    <div className="text-3xl font-black text-foreground">500+</div>
                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">Selections</div>
                  </motion.div>
                  
                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.75 }} className="glass-strong rounded-3xl p-5 flex flex-col justify-center items-center text-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 to-primary-glow/20 mb-3">
                      <Video className="h-6 w-6 text-primary" />
                    </div>
                    <div className="text-3xl font-black text-foreground">1000+</div>
                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">Lectures</div>
                  </motion.div>

                  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.85 }} className="glass-strong col-span-2 flex items-center justify-between rounded-3xl p-5">
                    <div>
                      <div className="text-base font-bold">Daily DPP & Mock Tests</div>
                      <div className="text-sm font-medium text-muted-foreground mt-0.5">Subject-wise practice</div>
                    </div>
